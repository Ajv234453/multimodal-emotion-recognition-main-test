...
class RAVDESS(data.Dataset):
    ...
    def __getitem__(self, index):
        default_video = torch.ones(3, 10, 224, 224)  # 假设视频数据是3通道，10帧，每帧大小为224x224
        default_audio = torch.ones(10, 20)  # 假设音频特征是10个MFCC系数，每个系数有20个时间步
        default_road = torch.ones(3, 10, 224, 224)  # 假设道路数据格式与视频相同
        clip = default_video
        ...
        try:
            ...
            if self.data_type == 'video' or self.data_type == 'audiovisualroad':
                ...
                if clip is not None:
                    ...
                    clip = [ToTensor()(img) for img in clip]  # 确保每个图像都被转换为Tensor
            ...
            if self.data_type == 'road' or self.data_type == 'audiovisualroad':
                ...
                if road_frames is not None:
                    ...
                    road_frames = [ToTensor()(img) for img in road_frames]  # 确保每个图像都被转换为Tensor
