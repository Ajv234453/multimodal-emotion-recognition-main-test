...
def val_epoch_multimodal(epoch, data_loader, model, criterion, opt, logger,modality='both',dist=None ):
    ...
    model.eval()
    #评估模式,模型中的dropout和batch normalization层都会被固定
    ...
    top5 = AverageMeter()
    end_time = time.time()
    for i, (inputs_audio, inputs_visual,inputs_road,targets) in enumerate(data_loader):
        #print(i)
        ...
        inputs_road = inputs_road.permute(0,2,1,3,4)
        inputs_road = inputs_road.reshape(inputs_road.shape[0]*inputs_road.shape[1], inputs_road.shape[2], inputs_road.shape[3], inputs_road.shape[4])

        targets = targets.to(opt.device)
        with torch.no_grad():
            inputs_visual = Variable(inputs_visual)
            inputs_audio = Variable(inputs_audio)
            inputs_road = Variable(inputs_road)
            targets = Variable(targets)
        outputs = model(inputs_audio, inputs_visual, inputs_road)
        ...
        end_time = time.time()
        with open(os.path.join(opt.result_path, 'validation_process.txt'), 'a') as f:
            f.write(
                f'Epoch: {epoch},batch:{i + 1},iterm:{(epoch - 1) * len(data_loader) + (i + 1)},Loss: {losses.avg:.4f}, Prec@1: {top1.avg:.2f},Prec@5: {top5.avg:.2f}%\n')
